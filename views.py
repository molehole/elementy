...
def index(request):
    return render(request, 'elementy/index.html', {'dane': SAPImport.objects.filter(ackDate=None)})

def import_danych(request):
    linie = []
    pattern = r'^[a-zA-Z\W]'    
    with open('export_temp', 'br') as f:
        spamreader = csv.reader(f, delimiter=';', encoding='utf-8', errors='ignore')
    ...
    for each in linie:
        ...
        if empty_line is None:            
            ...
            SAPImport.objects.get_or_create(ta=each[0],
                material = E,
                name = each[2],
                scheduledDate = datetime.strptime(each[3], '%d.%m.%Y'),
                createDate = datetime.strptime(each[4], '%d.%m.%Y'),
                createTime = datetime.strptime(each[5], '%H:%M:%S'),
                ackDate = ackDate,
